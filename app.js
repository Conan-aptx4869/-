wx.setStorageSync("https", "https://shop-api.fanzhuanjia.com/");
...
App({
        ...
        uploadimg: function(data){
                ...
                wx.uploadFile({
                        ...
                        name: 'head',//这里根据自己的实际情况改
                        formData: null,//这里是上传图片时一起上传的数据
                        success: (res) => {
                                success++;//图片上传成功，图片上传成功的变量+1
                                var data = JSON.parse(res.data);
                                imgSrc.push(data.data.list);
                                ...
                                var picList = flatten(imgSrc);
                                var result = [];
                                var obj = {};
                                for (var i = 0; i < picList.length; i++) {
                                        if (!obj[picList[i].id]) {
                                                result.push(picList[i]);
